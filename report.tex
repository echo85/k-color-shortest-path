...
\abstract{
    ...
    dynamic programming (BDP) approach to solve the $k$-CSPP as an alternative to existing 
    ...
    standard random and grid graphs. On newly generated labyrinth instances 
    specifically designed to challenge existing algorithms, BDP significantly outperforms both 
...
The project is focused on the optimization of the k-color Shortest Path Problem \cite{Ferone2019} (k-CSPP) ,
which is a variant of the classic Shortest Path Problem \cite{dijkstra1959} (SPP) that we know
...
that can appear in the path is fixed to be $k$.
Considering a digraph $D = (\mathcal{V},A)$ and a set of colors $C$, every arc $(i,j)$ has a positive cost $d_{ij}$ and color $c_{ij} \in C$.
...
and a Branch \& Cut procedure with new Valid Inequalities \cite{Castro2024} that seems to perform better on new layered digraph generated challenging instances.

The goal of this project is to evaluate a possible alternative algorithm to face the k-CSPP which is an NP-Hard combinatorial problem 
by extending the Dynamic Programming and using a Bounded Bi-directional Dynamic Programming (BDP) approach (Righini et al. 2006 \cite{righini2006}). 

We will evaluate the limits of this technique across different benchmark instances already used in the literature (Table \ref{tab:ferone-instances}) and
on a newly generated set of instances designed to challenge existing solutions (Table \ref{tab:unified-instances}).

The proposed BDP approach maintained competitive execution times on standard random and grid digraphs (Table \ref{tab:numerical_results_ferone}), 
while on the newly generated labyrinth instances, BDP outperformed both mono-directional DP and Valid Inequalities Branch-and-Cut methods (Table \ref{tab:numerical_results_new}).  

The evaluation successfully mapped the boundaries of the approach, confirming that layered-based digraphs remain 
challenging for dynamic programming techniques due to the combinatorial explosion (Table \ref{tab:numerical_results_castro}).
...
Let $G$ be a digraph $D = (\mathcal{V}, A)$, with $|\mathcal{V}|$ nodes and $|A|$ arcs. Given two functions $C : A \to \mathbb{Z}^+$ and
$d : A \to \mathbb{R}^+$: the first is an arc-coloring function,
...
We define $\mathcal{C}= \cup_{a \in A} \{C(a)\}$ as the set containing all the colors
of $G$. For each color $h \in \mathcal{C}$, we define $A_h = \{a \in A \text{ s.t. } C(a) = h\}$ as the
set containing all the arcs with colors equal to $h$.

The k-CSPP consists of finding a minimum distance path
...
Second, $\forall h \in \mathcal{C}$, let $y_h$ be a binary decision variable to determine whether a color $h$ is in the solution ($y_h = 1$) or not ($y_h = 0$).
\begin{alignat}{3}
    \text{(FFP)}
...
& y_h \in \{0, 1\}, && \forall h \in \mathcal{C}. \label{eq:y_binary}
\end{alignat}
...
In this section, we propose a bi-directional dynamic programming approach to solve the k-CSPP,
...
The exploration is based on the label correcting\footnote{label correcting maintains mutable labels that are progressively updated and pruned via dominance rules and bounding constraints until the search space converges} technique; the DP explores
...
$$N(k) = \sum_{l=1}^{k} \binom{|\mathcal{C}|}{l}$$
...
The ($k$-CSPP) is defined on a digraph where the distance function assigns a non-negative distance to each arc, therefore, the shortest path will naturally be a simple (elementary) path.
...
Because the problem assumes non-negative arc weights, any potential cycles formed during the extension of the dominant path $P_{si}$ can be naturally truncated. 

\subsection{Bi-Directional Solution}
In the bi-directional dynamic programming (BDP) approach, the labels are extended both forward from node $s$ to
its successors and backward from node $t$ to its predecessors. 
...
This limitation, ensures that the bi-directional algorithm doesn't produce as many labels as the mono-directional one.
...
\WHILE{$E \neq \emptyset$}
    ...
    \FORALL{$L_i = (i, d_i, C_i) \in \bar{\Gamma}_i^{fw}$}
        \FORALL{$j \in \mathcal{V} : (i,j) \in A_r$}
                \STATE $L_j \leftarrow Extend(d_i + w_{ij}, C_i \cup \{C([i,j])\}, k_{max}, h_j^{fw}, UB)$
    ...
    \FORALL{$L_i = (i, d_i, C_i) \in \bar{\Gamma}_i^{bw}$}
        \FORALL{$j \in \mathcal{V} : (j,i) \in A_r$}
                \STATE $L_j \leftarrow Extend(d_i + w_{ji}, C_i \cup \{C([j,i])\}, k_{max}, h_j^{bw}, UB)$
...
The BDP Algorithm \ref{alg:bidirectional} is a variation of the method proposed by Righini et al. (2006) \cite{righini2006}.
First, a graph reduction $A_r$ (GRA \cite{CerroneRusso2023}) and an upper bound are calculated (Line 2).
For each node $i \in \mathcal{V}$, we indicate with $\Gamma_i$ the set of labels associated with the node, with $\bar{\Gamma}_i \subseteq \Gamma_i$ being the
subset of labels not extended so far. $E$ is the set of nodes to be explored;
$Extend$ is the extension procedure from $i$ to $j$. This procedure checks the color constraints and applies bounding for fathoming. 
$AddLabel$ is the procedure that inserts the new label $L$ into the set $\Gamma$ by applying dominance rules (Algorithm \ref{alg:addlabel}).
...
As Lower Bound (LB) we check if the cost of the label plus the lower bound calculated as the shortest path to the target node ($t$ or $s$) without constraints is greater than or equal to the calculated upper bound (UB), that is, $Cost(Label) + LB \geq UB$.
...
A drawback of this choice is that if the CCDA cannot find any upper bound, the bi-directional approach will double its exploration w.r.t the mono-DP approach.
A future improvement could try to mitigate this issue using alternative heuristics or bounding techniques such as arc-bounding.
\subsection{Join}
...
models for CPLEX 22.1. The time limit for each instance is set to 600 s.
...
algorithm, which eliminates arcs proven to be unnecessary (denoted as $|R(A)|$ in Table \ref{tab:ferone-instances} e \ref{tab:unified-instances}) for the
...
R3-27005 & 75\,000 & 750\,000 & 25      & 6 & 198 &
  G4-27004 & 62\,500 & 498\,500 & 1159 & 774 & 23746 \\
R3-27007 & 75\,000 & 750\,000 & 38      & 6 & 231 &
  G4-27005 & 62\,500 & 498\,500 & 883 & 773 & 23610 \\
R3-27008 & 75\,000 & 750\,000 & 15      & 7 & 196 &
  G4-27006 & 62\,500 & 498\,500 & 1519 & 762 & 23904 \\
R3-27010 & 75\,000 & 750\,000 & 178     & 5 & 246 &
  G4-27007 & 62\,500 & 498\,500 & 975 & 759 & 23995 \\
R3-27012 & 75\,000 & 750\,000 & 28      & 7 & 245 &
  G4-27009 & 62\,500 & 498\,500 & 792 & 764 & 23940 \\
\bottomrule
\end{tabular}
\end{table}
...
In Table \ref{tab:numerical_results_ferone}, we show the numerical results for the benchmark instances for random and grid digraph using the Dynamic Programming procedure with the $A^*$ extraction policy (Ferone et al. 2019 \cite{Ferone2021}), 
CPLEX solver with the CCDA \cite{CerroneRusso2023} heuristic solution
as a cutoff value, our implementation of the FFP model restricted to valid inequalities (17), (18), and (19) as proposed by Castro et al. (2024)\cite{Castro2024}) and finally our BDP solution.
...
The (-) indicates the algorithm reached the time limit or ran out of memory.
...
The layered-based digraphs (Table \ref{tab:numerical_results_castro})
both the DP and BDP algorithms suffer from combinatorial explosion due to the topology of this type of the graph and the $k$ which is set as the MCPP, this forces the process to explore all 
combinations to the ends, pruning very few labels at each time.
...
BDP enters the labyrinth from both sides simultaneously and only does half of the work on the optimal path.

The IBM CPLEX with the fixed $\textit{ub}$, turns out to be a bit slower than DP and BDP in the random graph on average, but 
has results in a fastest execution on grid digraph. Regarding the layered-based digraph were all solved to optimality with an average time of 26.46 seconds.
The new instances have been resolved, but as the graph grows in the depth, CPLEX starts to performs badly, because
...
The strongest valid inequalities state that the sum of the flow on arcs of color $h$ in a certain cut or non-reachable subset must be $\le y_h$ (the color activation variable). 
...
\midrule
Average & 236.2 & \textbf{0.0208} & 0.0302 & 0.5681 & 0.4991 \\
Median & 239.0 & \textbf{0.0011} & 0.0094 & 0.1179 & 0.0180 \\
Max & 333.0 & \textbf{0.1189} & 0.1373 & 2.4469 & 2.9082 \\
Min & 152.0 & \textbf{0.0010} & 0.0057 & 0.0952 & 0.0146 \\
\midrule
...
\midrule
Average & 227.0 & \textbf{0.0011} & 0.0059 & 0.1201 & 0.0173 \\
Median & 238.0 & \textbf{0.0011} & 0.0058 & 0.1187 & 0.0174 \\
Max & 246.0 & \textbf{0.0013} & 0.0061 & 0.1366 & 0.0180 \\
Min & 196.0 & \textbf{0.0011} & 0.0057 & 0.1069 & 0.0166 \\
\midrule
G1--27001 & 6233 & 0.0002 & 0.0007 & 0.0073 & 0.0018 \\
G1--27003 & 6203 & 0.0026 & 0.0011 & 0.0135 & 0.0045 \\
G1--27004 & 6375 & 45.2761 & 0.0016 & 0.0082 & 0.0027 \\
...
\midrule
Average & 6230.7 & 7.5468 & \textbf{0.0010} & 0.0086 & 0.0025 \\
Median & 6200.0 & \textbf{0.0007} & 0.0009 & 0.0077 & 0.0021 \\
Max & 6375.0 & 45.2761 & \textbf{0.0016} & 0.0135 & 0.0045 \\
Min & 6183.0 & \textbf{0.0002} & 0.0007 & 0.0072 & 0.0017 \\
\midrule
...
Median & 23927.0 & 0.1367 & 0.0383 & 0.1297 & \textbf{0.0247} \\
Max & 23995.0 & 16.3234 & 127.8307 & 0.1595 & \textbf{0.0376} \\
Min & 23798.0 & 0.0273 & \textbf{0.0162} & 0.1049 & 0.0185 \\
\bottomrule
\end{tabular*}
\end{table}
...
Median & 27388.0 & - & - & 23.8463 & \textbf{2.3121} \\
Max & 51145.0 & - & - & 59.0637 & \textbf{13.7712} \\
Min & 27264.0 & - & - & 2.9353 & \textbf{1.6920} \\
\bottomrule
\end{tabular*}
\end{table}
...
Median  & 360.0 & 0.0261 & \textbf{0.0001} & 6.1689 & 5.6575 \\
Max     & 560.0 & 0.0261 & \textbf{0.0003} & 377.3205 & 42.2813 \\
Min     & 260.0 & 0.0261 & \textbf{0.0001} & 0.9947 & 1.6116 \\
\bottomrule
\end{tabular*}
\end{table}

\section{Conclusions and future works}
This project proposed and evaluated a bounded bi-directional dynamic programming (BDP) \cite{righini2006} approach to solve the NP-hard $k$-color Shortest Path Problem ($k$-CSPP). 
...
Computational experiments (Table \ref{tab:numerical_results_ferone}) demonstrate that BDP maintains competitive execution times on standard random and grid digraphs. 
Furthermore, on newly generated labyrinth instances BDP significantly outperformed (Table \ref{tab:numerical_results_new}) both mono-DP and Valid Inequalities by reducing 
the required search space and quickly identifying path infeasibilities.  
Despite these improvements, layered-based digraphs remain exceptionally challenging (Table \ref{tab:numerical_results_castro}) for dynamic programming techniques due to severe combinatorial explosion. 

Future improvements could include integrating other bounding techniques like State Space Relaxation \cite{Christofidesetal1981} or Decremental State Space Relaxation \cite{righini2006} by forcing the algorithm 
to only track a subset of the most frequently used colors. This could help to reduce the exploration,
even though the non-additive property of the colors is an important limit to take into account. 

Additionally, the development of a hybrid approach that combines bi-directional dynamic programming 
with Valid Inequalities. 
Since Valid Inequalities \cite{Castro2024} demonstrated strong performance and stability across layered digraphs (Table \ref{tab:numerical_results_castro}), 
integrating them to compute tighter bounds could 
mitigate the combinatorial explosion that limits DP-based techniques on those specific topologies.
